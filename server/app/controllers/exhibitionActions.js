...
const add = async (req, res, next) => {
  ...
  try {
    ...
    res
      .status(201)
      .send(`Exposition ajoutée avec succès. ID : ${result.insertId}`);
  } catch (error) {
    next(error);
  }
};
...
const readArtwork = async (req, res, next) => {
  try {
    const exhibition = await tables.exhibition.readExhibitionArtwork(
      req.params.id
    );
    if (exhibition == null) {
      res.sendStatus(404);
...
const addArtwork = async (req, res, next) => {
  ...
  try {
    const result =
      await tables.exhibition.createExhibitionArtwork(artworkExhibition);
    res
      .status(201)
      .send(`Exposition ajoutée avec succès. ID : ${result.insertId}`);
  } catch (error) {
...
const destroyArtwork = async (req, res, next) => {
  ...
  try {
    const affectedRows = await tables.exhibition.deleteExhibitionArtwork(
      exhibitionId,
      artworkId
    );
    if (affectedRows === 0) {
      return res
        .status(404)
        .send("Aucune correspondance trouvée pour les IDs fournis.");
    }
